...
After you are modifying the code, you can use the same command (`docker-compose up`) to render the webpage with all you changes. 
### Running in Local
To get start, make sure you have installed [Ruby](https://www.ruby-lang.org/en/downloads/) and [Bundler](https://bundler.io/).
- It is recommended to use [rbenv](https://github.com/rbenv/rbenv) to manage the ruby version (brew install ruby might mess up the version).
- For Mac User:
```bash
brew install rbenv ruby-build
```
...
rbenv init
```
- Then, you can use rbenv to install and manage your ruby version. Before install, maybe you need to check if you have the [dependencies](https://github.com/rbenv/ruby-build/wiki#suggested-build-environment)
```bash
...
```
## Local Running
After you have installed [Ruby](https://www.ruby-lang.org/en/downloads/) and [Bundler](https://bundler.io/), go to the repo directory, and install corresponding bundle packages
```bash
cd metadriverse-web
bundle install
bundle exec jekyll serve --lsi
...
- dropdown.md: Determine what will be showed in the dropdown menu.
